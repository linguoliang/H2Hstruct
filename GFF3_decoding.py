...
class Isoform:
    ...
    def builtIntron(self):
        ...
        self.Introns = [[self.Introns[2 * i] + 1, self.Introns[2 * i + 1] - 1] for i in range(floor(len(self.Introns) / 2))]
        self.Introns.pop()
        self.Introns.pop(0)
...
class GeneSubunit(Gene):
    ...
    def builtsuperexon(self):
        ...
        self.exon = [[int(self.superIsoform[2 * i]), int(self.superIsoform[2 * i + 1])] for i in
                     range(floor(len(self.superIsoform) / 2)) if True]
    ...
    def builtsuperIsoform(self):
        ...
        self.CommonIntrons = [[superIsoform[2 * i] + 1, superIsoform[2 * i + 1] - 1] for i in
                              range(floor(len(superIsoform) / 2)) if True]
...
def decodegff(gtffilename):
    ...
    for key, val in list(genomeDict.items()):
